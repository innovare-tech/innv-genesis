...
export class AppInitializer<T extends INestApplication = INestApplication> {
  ...
  private onAfterInitializeBlock?: (app: INestApplication) => void;
  ...
  public onAfterInitialize(block: (app: INestApplication) => void): this {
    this.onAfterInitializeBlock = block;
  ...
  private async listen(): Promise<void> {
    ...
    if (this.swaggerOptions) {
      ...
      if (this.swaggerOptions.bearerAuth) {
        documentBuilder = documentBuilder.addBearerAuth(
          this.swaggerOptions.bearerAuth.scheme,
          this.swaggerOptions.bearerAuth.name,
        );
      }

      for (const apiKey of this.swaggerOptions.apiKeys ?? []) {
    ...
    if (this.onAfterInitializeBlock) {
      this.onAfterInitializeBlock(this.app);
    }
  }
}
