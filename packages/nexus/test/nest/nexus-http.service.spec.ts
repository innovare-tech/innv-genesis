    ...
    describe('request method', () => {
        it('should call axios.request with correct parameters', async () => {
            const mockAxiosResponse = {
                data: { id: 1, name: 'Test' }, status: 200, headers: { 'content-type': 'application/json' },
            };
            ...
            expect(mockRequest).toHaveBeenCalledWith(expect.objectContaining({
                baseURL: mockClientOptions.baseUrl, url: mockRequestConfig.url, method: mockRequestConfig.method,
            }));
        });
        ...
        it('should return NexusHttpResponse on successful request (2xx)', async () => {
            ...
            const mockAxiosResponse = {
                data: responseData, status: 201, headers: { location: '/users/1' },
            };
            mockRequest.mockResolvedValue(mockAxiosResponse);
            const result = await service.request<typeof responseData>(mockClientOptions, mockRequestConfig);
        ...
        it('should throw NexusHttpError with isAxiosError=false and code for non-axios errors', async () => {
            ...
            mockIsAxiosError.mockImplementation(
                (payload: any): payload is AxiosError => payload === genericError ? false : (typeof payload === 'object' && payload !== null && payload.isAxiosError === true)
            );
