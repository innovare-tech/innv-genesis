...
import { MockValidationErrorDto, MockBodyDto } from '../helpers/mock-dtos';
...
describe('createNexusClientProvider', () => {
  ...
  const setupTestBed = async (clientClass: any, provider: Provider) => {
    moduleRef = await Test.createTestingModule({
      providers: [provider, mockHttpServiceProvider, mockConfigServiceProvider],
    }).compile();
  ...
  beforeEach(async () => {
    ...
    hardcodedApiClient = await setupTestBed(
      MockHardcodedApiClient,
      hardcodedProvider,
    );
  ...
  describe('Request Assembly and Execution', () => {
    it('should call httpService.request with correctly assembled GET request (hardcoded config)', async () => {
      mockHttpServiceRequest.mockResolvedValue({
        data: {},
        status: 200,
        headers: {},
      });
      ...
      expect(mockHttpServiceRequest).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          method: 'GET',
          url: '/items/123?details=true',
          headers: { 'X-Static': 'StaticValue', 'X-Tenant-Id': 'tenant-abc' },
        }),
      );
    });
  ...
  describe('Response Handling', () => {
    ...
    it('should return Err(validatedData) for mapped error responses (@ErrorResponse)', async () => {
      ...
      axiosError.response = mockAxiosErrorResponse;
      axiosError.isAxiosError = true;
      axiosError.code = 'ERR_BAD_REQUEST';
      mockHttpServiceRequest.mockRejectedValue(axiosError);
      const result = await hardcodedApiClient.createItem({
        description: '',
        value: -1,
      });
    ...
    it('should re-throw error for unmapped error statuses', async () => {
      ...
      axiosError.response = mockAxiosErrorResponse;
      axiosError.isAxiosError = true;
      axiosError.code = 'ERR_INTERNAL_SERVER_ERROR';
      mockHttpServiceRequest.mockRejectedValue(axiosError);
      await expect(hardcodedApiClient.getItem(1)).rejects.toThrow(
        '[Nexus] Erro não tratado em MockHardcodedApiClient.getItem: Server Error',
      );
    });
    ...
    it('should re-throw error for network/timeout errors', async () => {
      ...
      mockHttpServiceRequest.mockRejectedValue(axiosError);
      await expect(hardcodedApiClient.getItem(1)).rejects.toThrow(
        '[Nexus] Erro não tratado em MockHardcodedApiClient.getItem: Network Error',
      );
    });
  });
  ...
  describe('Configuration Resolution Errors', () => {
    it('should throw if baseUrlEnvKey is used but ConfigService is missing', async () => {
      ...
      await expect(async () => {
        moduleRef = await Test.createTestingModule({
          providers: [provider, mockHttpServiceProvider],
        }).compile();
