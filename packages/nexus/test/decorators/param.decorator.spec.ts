...
describe('Parameter Decorators', () => {
    ...
    it('should define metadata correctly for multiple parameters', () => {
        ...
        expect(finalMetadataArray).toEqual(
            expect.arrayContaining([
                expect.objectContaining<ApiParamMeta>({ index: 0, type: 'path', name: 'id' }),
                expect.objectContaining<ApiParamMeta>({ index: 1, type: 'query', name: 'limit' }),
                expect.objectContaining<ApiParamMeta>({ index: 2, type: 'body', name: undefined }),
            ])
        );

        // Opcional: Verifica se os metadados finais foram salvos corretamente
        const savedMeta = Reflect.getMetadata(API_PARAM_META_KEY, TestController.prototype, 'testMethod');
    ...
    it('should throw error when used on constructor parameter', () => {
        ...
        expect(() => {
            class TestController {
                constructor(@Query('invalid') param: string) { console.log(param); }
            }
            // @ts-ignore
